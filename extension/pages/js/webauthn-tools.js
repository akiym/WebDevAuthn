...
window.authnTools = {
  ...
  isBase64any: function (test) {
    return new RegExp(
      /^(?:[A-Za-z0-9_\-+/]{4})*(?:[A-Za-z0-9_\-+/]{2}={0,2}|[A-Za-z0-9_\-+/]{3}={0,1})?$/,
    ).test(test)
      ? true
      : false
  },
  ...
  isBase64: function (test) {
    return new RegExp(
      /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}={0,2}|[A-Za-z0-9+/]{3}={0,1})?$/,
    ).test(test)
      ? true
      : false
  },
  ...
  uint8ArrayToInt: function (array, length = 0) {
    ...
    if (length == 1 || length == 2 || length == 4) return dataView['getUint' + length * 8]()
    else throw 'Error length ' + length + ' not supported.'
  },
      ...
      try {
        ...
        doc.write(
          '<body>' +
            ...
            '</style>' +
            '<div id="wrapper">' +
            new URL(window.location.href || 'http://website').host +
            ' says<br>' +
            '<div id="text"></div>' +
