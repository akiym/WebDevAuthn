...
window.AuthnDevice = (function (localURL) {
  ...
  AuthnDevice.prototype._updateCounter = function () {
    if (this.counterIncreaseEveryMs > 0) {
      ...
      this.sign_count =
        Math.floor((new Date().getTime() - this.createdAt) / this.counterIncreaseEveryMs) + 1
    } else {
      this.sign_count++
    }
  }
  ...
  AuthnDevice.prototype._getSignCount = function () {
    ...
    if (Number.isInteger(this.testing.freezeSigCounter)) return this.testing.freezeSigCounter
    // Return counter
    ...
    if (!this.derivedKey) {
      ...
      if (!this.masterkey) {
        if (this.askmasterkey) {
          ...
          if (!this.masterkey || this.masterkey.length < 1) throw new Error('No password given')
        } else {
  ...
  AuthnDevice.prototype._cred_init = async function (
    rp_id,
    user_handle = null,
    key_id = null,
    alg = null,
  ...
  AuthnDevice.prototype._cred_init_with_ResidentKey = async function (
    rp_id,
    user_handle = null,
    key_id = null,
  ) {
  ...
  AuthnDevice.prototype.create = async function (options, _origin) {
    ...
    if (algs.length < 1)
      throw new Error('Requested pubKeyCredParams does not contain supported type')
    let keyPairAlg = algs[0].alg

    // Check if required Resident Key aka Discoverable credential
    let requestResidentKey =
    ...
    let origin = (() => {
      if (_origin)
        try {
          return new URL(_origin).origin
        } catch (e) {}
      if (this.localUrl)
        try {
          return new URL(this.localUrl).origin
        } catch (e) {}
      if (options.publicKey && options.publicKey.rp && options.publicKey.rp.id)
        try {
          return new URL('https://' + options.publicKey.rp.id).origin
        } catch (e) {}
    ...
    let rpid = (() => {
      if (options.publicKey && options.publicKey.rp && options.publicKey.rp.id)
        try {
          return new URL('https://' + options.publicKey.rp.id).hostname
        } catch (e) {}
      if (this.localUrl)
        try {
          return new URL(this.localUrl).hostname
        } catch (e) {}
    ...
    let rp_id_hash = new Uint8Array(
      await window.crypto.subtle.digest('SHA-256', new TextEncoder().encode(this.rp_id)),
    )
    ...
    if (
      ...
      (options.publicKey.attestation == 'indirect' || options.publicKey.attestation == 'direct')
    ) {
      // Generate signature
  ...
  AuthnDevice.prototype.get = async function (options, _origin) {
    ...
    let origin = (() => {
      ...
      if (options.publicKey && options.publicKey.rpId)
        try {
          return new URL('https://' + options.publicKey.rpId).origin
        } catch (e) {}
      return new URL(localURL).origin
    })()
    ...
    let rpid = (() => {
      if (options.publicKey && options.publicKey.rpId)
        try {
          return new URL('https://' + options.publicKey.rpId).hostname
        } catch (e) {}
      if (this.localUrl)
        try {
          return new URL(this.localUrl).hostname
  ...
  let ans1 = {
    ...
    objectToDER: function (obj) {
      ...
      // Strings are used from multiple types in the form of `TYPE{VALUE}`
        ...
        if (obj.match(/^OBJECT-IDENTIFIER{[0-9.]+}$/i)) {
          ...
          let value = obj.split('').map(function (c) {
            return c.charCodeAt(0)
          })
      ...
      // Integer numbers encoding
        ...
        return this.concatUint8Array(
          new Uint8Array([0x02, ...this.intToLength(value.length)]),
          value,
        )
      }
    ...
    parseSignatureECDSA: function (signature) {
      ...
      let signHex = Array.prototype.map
      ...
      let signatureDER = new Uint8Array(
        Array.from(der.match(/.{2}/g)).map((h) => {
    ...
    generateCertificate: async function (info, publicKey, caPrivateKey) {
      ...
      let tbs = this.objectToDER([
        ...
        [
          ...
          // organizationName
          new Set([
            ['OBJECT-IDENTIFIER{2.5.4.10}', 'UTF8{' + info.issuedBy.organizationName + '}'],
          ]),
          // commonName
          new Set([['OBJECT-IDENTIFIER{2.5.4.3}', info.issuedBy.commonName]]),
        ],
        // Dates
        [new Date('2021-01-01T00:00:00.000+02:00'), new Date('2121-01-01T00:00:00.000+02:00')],
        // Subject field
        [
          // countryName
          ...
          // organizationName
          new Set([
            ['OBJECT-IDENTIFIER{2.5.4.10}', 'UTF8{' + info.issuedTo.organizationName + '}'],
          ]),
          // organizationalUnitName
          new Set([
            ['OBJECT-IDENTIFIER{2.5.4.11}', 'UTF8{' + info.issuedTo.organizationalUnitName + '}'],
          ]),
          // commonName
      ...
      let signature = new Uint8Array(
        await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, caPrivateKey, tbs),
      )
      ...
      let certificate = this.objectToDER([
        ...
        this.tagDER(
          'BIT-STRING',
          this.concatUint8Array(new Uint8Array([0x00]), this.parseSignatureECDSA(signature)),
        ),
      ])
    ...
    let VirtualAuthn = {
      ...
      override_api: function () {
        ...
        credentials.create = function () {
          return (
            VirtualAuthn.virtual ? VirtualAuthn.virtual_create : VirtualAuthn.original_create
          ).apply(this, arguments)
        }
        credentials.get = function () {
          return (
            VirtualAuthn.virtual ? VirtualAuthn.virtual_get : VirtualAuthn.original_get
          ).apply(this, arguments)
        }
      },
