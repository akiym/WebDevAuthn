...
window.authnGet = {
  ...
  analyzeAuthenticatorData: function (authData) {
    ...
    if (bufferSize >= 32 + 1 + 4 + 16) {
      ...
      authenticatorData.attestedCredentialData.credentialIdLength =
        window.authnTools.uint8ArrayToInt(authData.slice(32 + 1 + 4 + 16, 32 + 1 + 4 + 16 + 2))
  ...
  checkOptions: function () {
    ...
    ;(() => {
      ...
      let userVerification = this.options.publicKey.hasOwnProperty('userVerification')
        ? this.options.publicKey.userVerification
        : 'preferred'
      let recomended =
    ...
    if (this.options.publicKey.hasOwnProperty('rpId')) {
      if (!isUSVString(this.options.publicKey.rpId)) {
        observations.push({
          message: '"publicKey.rpId" does not have a DOMString type value.',
          type: 'danger',
        })
      }
    }
    ...
    if (!this.options.publicKey.hasOwnProperty('allowCredentials')) {
      observations.push({
        message: '"publicKey.allowCredentials" was not set, default value is [].',
        type: 'info',
      })
    } else {
      for (let i = 0; i < this.options.publicKey.allowCredentials.length; i++) {
        ...
        if (!this.options.publicKey.allowCredentials[i].hasOwnProperty('type')) {
          observations.push({
            message: '"publicKey.allowCredentials[' + i + '].type" was not found.',
            type: 'danger',
          })
        } else if (!isDOMString(this.options.publicKey.allowCredentials[i].type)) {
          observations.push({
            message:
              '"publicKey.allowCredentials[' + i + '].type" does not have a DOMString type value.',
            type: 'danger',
          })
          ...
          observations.push({
            message:
              ...
              this.options.publicKey.allowCredentials[i].type +
              '".',
            type: 'danger',
          })
        }
        ...
        if (!this.options.publicKey.allowCredentials[i].hasOwnProperty('id')) {
          observations.push({
            message: '"publicKey.allowCredentials[' + i + '].id" was not found.',
            type: 'danger',
          })
        } else if (!isBufferSource(this.options.publicKey.allowCredentials[i].id)) {
          observations.push({
            message:
              '"publicKey.allowCredentials[' + i + '].id" does not have a BufferSource type value.',
            type: 'danger',
          })
        } else if (this.options.publicKey.allowCredentials[i].id.byteLength < 16) {
          observations.push({
            message: '"publicKey.allowCredentials[' + i + '].id" size is less than 16 bytes',
            type: 'danger',
          })
        }
            ...
            for (let j = 0; j < this.options.publicKey.allowCredentials[i].transports.length; j++) {
              if (!isDOMString(this.options.publicKey.allowCredentials[i].transports[j])) {
                ...
                !['usb', 'nfc', 'ble', 'internal'].includes(
                  this.options.publicKey.allowCredentials[i].transports[j],
                )
              ) {
                observations.push({
                  message:
    ...
    if (this.options.publicKey.hasOwnProperty('userVerification')) {
      if (!isDOMString(this.options.publicKey.userVerification)) {
        observations.push({
          message: '"publicKey.userVerification" does not have a DOMString type value.',
          type: 'danger',
        })
      } else if (
        !['required', 'preferred', 'discouraged'].includes(this.options.publicKey.userVerification)
      ) {
        observations.push({
          message: '"publicKey.userVerification" is set to an unknown value.',
          type: 'danger',
        })
      }
    } else {
      observations.push({
        message: '"publicKey.userVerification" was not set, default value is "preferred".',
        type: 'info',
      })
    }
    ...
    if (
      ...
      observations.push({
        message: '"publicKey.userVerification" is adviced to be set to "required".',
        type: 'warning',
      })
    }
        ...
        if (this.options.publicKey.extensions.hasOwnProperty(extension)) {
          ...
          if (extension === 'appid') {
            if (!isUSVString(this.options.publicKey.extensions[extension])) {
              observations.push({
                message: '"publicKey.extensions.appid" does not have a USVString type value.',
                type: 'danger',
              })
            }
          }
          ...
          // Client to Authenticator Protocol (CTAP) > HMAC Secret Extension (hmac-secret)
            ...
            if (!this.options.publicKey.extensions[extension].hasOwnProperty('salt1')) {
              observations.push({
                message: '"publicKey.extensions.hmacGetSecret.salt1" was not found.',
                type: 'danger',
              })
            } else if (
              !(this.options.publicKey.extensions[extension].salt1 instanceof ArrayBuffer)
            ) {
              observations.push({
                message:
                  '"publicKey.extensions.hmacGetSecret.salt1" does not have an ArrayBuffer type value.',
                type: 'danger',
              })
            } else if (this.options.publicKey.extensions[extension].salt1.byteLength != 32) {
              observations.push({
                message: '"publicKey.extensions.hmacGetSecret.salt1" is not 32 bytes long.',
                type: 'danger',
              })
            }
            if (this.options.publicKey.extensions[extension].hasOwnProperty('salt2')) {
              if (!(this.options.publicKey.extensions[extension].salt2 instanceof ArrayBuffer)) {
                observations.push({
                  message:
                    '"publicKey.extensions.hmacGetSecret.salt2" does not have an ArrayBuffer type value.',
                  type: 'danger',
                })
              } else if (this.options.publicKey.extensions[extension].salt2.byteLength != 32) {
                observations.push({
                  message: '"publicKey.extensions.hmacGetSecret.salt2" is not 32 bytes long.',
                  type: 'danger',
                })
              }
            }
            ...
            for (let key in this.options.publicKey.extensions[extension]) {
              if (
                ...
                !['salt1', 'salt2'].includes(key)
              ) {
                observations.push({
                  message: 'unknown attribute "publicKey.extensions.hmacGetSecret.' + key + '"',
                  type: 'warning',
                })
              }
            }
          ...
          // WebAuthn-2 > Large blob storage extension (largeBlob)
            ...
            if (this.options.publicKey.extensions[extension].hasOwnProperty('support')) {
              ...
              observations.push({
                message:
                  '"publicKey.extensions.largeBlob.support" is only valid during registration.',
                type: 'danger',
              })
            }
          ...
          if (extension === 'txAuthSimple') {
            if (!isUSVString(this.options.publicKey.extensions[extension])) {
              observations.push({
                message:
                  '"publicKey.extensions.txAuthSimple" does not have a USVString type value.',
                type: 'danger',
              })
            }
            observations.push({
              message: '"publicKey.extensions.txAuthSimple" was removed from WebAuthn-2.',
              type: 'danger',
            })
          }
          // WebAuthn-1 > Generic Transaction Authorization Extension (txAuthGeneric)
            ...
            if (!this.options.publicKey.extensions[extension].hasOwnProperty('contentType')) {
              observations.push({
                message: '"publicKey.extensions.txAuthGeneric.contentType" was not found.',
                type: 'danger',
              })
            } else if (!isUSVString(this.options.publicKey.extensions[extension].contentType)) {
              observations.push({
                message:
                  '"publicKey.extensions.txAuthGeneric.contentType" does not have a USVString type value.',
                type: 'danger',
              })
            }
            ...
            if (!this.options.publicKey.extensions[extension].hasOwnProperty('content')) {
              observations.push({
                message: '"publicKey.extensions.txAuthGeneric.content" was not found.',
                type: 'danger',
              })
            } else if (
              !(this.options.publicKey.extensions[extension].content instanceof ArrayBuffer)
            ) {
              observations.push({
                message:
                  '"publicKey.extensions.txAuthGeneric.content" does not have a USVString type value.',
                type: 'danger',
              })
            }
            // Check for unknown keys
            for (let key in this.options.publicKey.extensions[extension]) {
              if (
                this.options.publicKey.extensions[extension].hasOwnProperty(key) &&
                !['contentType', 'content'].includes(key)
              ) {
                observations.push({
                  message: 'unknown attribute "publicKey.extensions.txAuthGeneric.' + key + '"',
                  type: 'warning',
                })
              }
            }
  ...
  renderNotes: function () {
    ...
    document.getElementById('credential-get-options-notes-notification').textContent =
  ...
  renderCredentials: function () {
    ...
    credential.response.userHandle = credential.response.userHandle
      ? window.authnTools.auto(credential.response.userHandle)
  ...
  initCredentials: function () {
    ...
    $('#credential-get-publickeycredentialdescriptor-add').click(() => {
      let i = document.getElementsByClassName(
        'credential-get-publickeycredentialdescriptor-select',
      ).length
...
;(function () {
  ...
  let externalRequestsHandler = function (origin, data, callback) {
    ...
    window.authnGet.crossSiteReponse = () => {
      callback({
        ...
        extensions: window.authnTools.serialize(window.authnGet.extensions || {}),
      })
    }
    window.authnGet.crossSiteErrorReponse = (error) => {
      callback({
        id: data.id,
  ...
  document.getElementById('credential-get-load-from-code').addEventListener(
    'click',
    () => {
