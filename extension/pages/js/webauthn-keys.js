...
;(function () {
  ...
  var reload = function () {
    ...
    for (let i = 0; i < credentials.length; i++) {
      ...
      modality.textContent = data
        ? 'client-side (Resident Key aka Discoverable credential)'
        : 'server-side (inside credential ID)'
      wrapper.appendChild(modality)
      wrapper.appendChild(document.createElement('br'))
  ...
  reload()
  document.getElementById('localstorage-keys-reload').addEventListener('click', () => {
    reload()
  })
...
;(function () {
  ...
  reload()
  document
    .getElementById('localstorage-authenticator-keys-reload')
...
;(function () {
  ...
  window.tryToDecodeWrappedKey = async function (keyid, salt) {
    ...
    document
      ...
      .getElementsByTagName('pre')[0].textContent = 'Decrypting ...'
    document
      .getElementById('decryptWrappedKeyView-decoded')
      .getElementsByTagName('pre')[0].textContent = 'Decrypting ...'

    let stored = await AuthnDevice._loadOrGenerateMasterKey().catch(function () {
      return null
    })
    if (!salt && stored) {
  ...
  let decodeWrappedKey = async function (
    keyid,
    masterkey = null,
    salt = null,
    iterations = 100000,
    ...
    if (!masterkey) {
      ...
      if (!masterkey || masterkey.length < 1) throw new Error('No password given')
    }
...
;(function () {
  ...
  storage.get('authn-masterkey').then(function (result) {
    if (result['authn-masterkey']) {
      document.getElementById('localstorage-authenticator-masterkey').value =
        result['authn-masterkey']
    }
  })
  // On update
  document
    .getElementById('localstorage-authenticator-masterkey')
    .addEventListener('change', function () {
      // If empty key, remove (will be auto-generated on next use)
      if (!this.value.length) {
        storage.remove('authn-masterkey')
        window.jsNotify.danger('MasterKey was reset. A new one will be generated on next use.', {
          time2live: 5 * 1000,
        })
      }
      // Update key
  ...
  document
    .getElementById('localstorage-authenticator-always-ask-for-masterkey')
    .addEventListener('change', function () {
      if (!this.checked) {
  ...
  document.getElementById('localstorage-authenticator-always-ask-for-masterkey').checked =
    window.localStorage.getItem('VirtualAuthn-always-ask-for-masterkey') == 'yes'

  let visibility = false
  document
    .getElementById('localstorage-authenticator-masterkey-visible')
    .addEventListener('click', function () {
      visibility = !visibility
      if (visibility) {
        document.getElementById('localstorage-authenticator-masterkey-visible').className =
          'fas fa-eye'
