...
window.WebDevAuthn =
  window.WebDevAuthn ||
  ((cWindow, credentials, PKCredential) => {
    let WebDevAuthn = {
      // Dev tools URL
      devDomain: '',
      ...
      init: function () {
        ...
        if (script) {
          if (script.dataset.devDomain || script.getAttribute('dev-domain'))
          ...
          if (
            ...
            (script.getAttribute('virtual') &&
              script.getAttribute('virtual').toLowerCase() == 'true')
          )
            this._virtual = true
          if (
            (script.dataset.development && script.dataset.development.toLowerCase() == 'true') ||
            (script.getAttribute('development') &&
              script.getAttribute('development').toLowerCase() == 'true')
          )
            this._development = true
          if (
            (script.dataset.pauseWithAlert &&
              script.dataset.pauseWithAlert.toLowerCase() == 'true') ||
            (script.getAttribute('pause-with-alert') &&
              script.getAttribute('pause-with-alert').toLowerCase() == 'true')
          )
            this._pauseWithAlert = true
          if (
            (script.dataset.instanceOfPubKey &&
              script.dataset.instanceOfPubKey.toLowerCase() == 'true') ||
            (script.getAttribute('instance-of-pub-key') &&
              script.getAttribute('instance-of-pub-key').toLowerCase() == 'true')
          )
            this._patchPubCred = true
          if (
            (script.dataset.debugger && script.dataset.debugger.toLowerCase() == 'true') ||
            (script.getAttribute('debugger') &&
              script.getAttribute('debugger').toLowerCase() == 'true')
          )
            this._debugger = true
          if (
            (script.dataset.platformAuthenticatorAvailable &&
              script.dataset.platformAuthenticatorAvailable.toLowerCase() == 'true') ||
            (script.getAttribute('platform-authenticator-available') &&
              script.getAttribute('platform-authenticator-available').toLowerCase() == 'true')
          )
        ...
        this.PKCredential = {
          ...
          isUserVerifyingPlatformAuthenticatorAvailable:
            PKCredential.isUserVerifyingPlatformAuthenticatorAvailable,
        }
      ...
      connect: function (instance, send = true) {
        let openWin = () => {
          return cWindow.open(
            this.devDomain + (instance.authn == 'create' ? this.devCreatePath : this.devGetPath),
          )
        }
        if (!instance.win) instance.win = openWin()
        if (!send) return
        // Send info
        ...
        let interval = setInterval(() => {
          ...
          if (tries > 15 * (1000 / ms)) {
            ...
            let url =
              this.devDomain +
              (instance.authn == 'create' ? this.devCreatePath : this.devGetPath) +
              '?data=' +
              encodeURIComponent(
                cWindow.btoa(
                  JSON.stringify({
            ...
            Popup('Copy custom analyser URL and paste back response', 'prompt', url, true).then(
              (data) => {
                // Decode data
                try {
      ...
      create: function () {
        if (
          !this._development ||
          arguments.length < 1 ||
          !arguments[0].hasOwnProperty('publicKey')
        ) {
          // Normal Call
          ...
          return new Promise((resolve, reject) => {
            ...
            //cWindow.focus();
            this.WebAuthn.create
              .apply(this.WebAuthn.scope, arguments)
              .then((credential) => {
                if (this._debugger) debugger
                //instance.credential = {
                //	id: credential.id,
      ...
      isUserVerifyingPlatformAuthenticatorAvailable: function () {
        ...
        // Else call original
        return this.PKCredential.isUserVerifyingPlatformAuthenticatorAvailable.apply(
          this.PKCredential.scope,
          arguments,
        )
      },
        ...
        return JSON.parse(jsonStr, function (key, value) {
          try {
            if (value.hasOwnProperty('flag') && value.flag === 'FLAG_TYPED_ARRAY') {
              if (value.constructor === 'ArrayBuffer') return new Uint8Array(value.data).buffer
    ...
    let VirtualPublicKeyCredential = function () {
      ...
      return class extends class Dummy {} {
        constructor(obj) {
          ...
          // Save extensions
          this[priv] = {}
          this[priv].extensions =
            typeof obj.getClientExtensionResults == 'function'
    ...
    let PopupFallback = function (text, type = 'alert', defaultText = '') {
      return new Promise((resolve, reject) => {
        try {
          ...
          doc.write(
            '<body>' +
              ...
              (type == 'confirm' || type == 'prompt'
                ? '<input id="ok-btn" class="btn" type="button" value="OK"/> <input id="cancel-btn" class="btn btn-invert" type="button" value="Cancel"/>'
          ...
          iframe.style.height =
            Math.ceil(Math.max(wrapper.clientHeight, wrapper.scrollHeight)) + 38 + 2 + 'px'
          ...
          if (type == 'confirm') {
            ...
            doc.getElementById('ok-btn').addEventListener(
              'click',
              () => {
                close(true)
              },
            ...
            doc.getElementById('cancel-btn').addEventListener(
              'click',
              () => {
                close(false)
              },
    ...
    let Popup = function (text, type = 'alert', defaultText, force = false) {
      let popup =
        type == 'confirm' ? cWindow.confirm : type == 'prompt' ? cWindow.prompt : cWindow.alert
      return new Promise((resolve, reject) => {
        try {
          let t = new Date()
          let v = force
